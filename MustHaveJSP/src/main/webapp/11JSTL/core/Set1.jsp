...
<!-- 
 ...
 -->
	...
	<!-- 위 4개의 변수는 scope 지정이 없으므로 가장 좁은 page영역에 지정된다. -->
	
	<!-- 속성명이 중복되지 않는다면 영역을 표시하는 내장객체를 생략할수 있다. -->
	<h4>EL을 이용해 변수 출력</h4>
	<ul>
		<li>dirctVar : ${ pageScope.directVar }</li>
		<li>elVar : ${ elVar }</li>
		<li>expVar : ${ expVar }</li>
	...
	<!-- 
		JSTL은 jsp코드이므로 value속성에 기술할때 객체를 생성하기위한 더블쿼테이션이 겹쳐지는
		경우가 발생하게 된다. 이때는 value를 
		싱글쿼테이션으로 감싸서 겹쳐지지 않게 처리해야 한다.
	 -->
	 <!-- 자바빈즈 생성시 생성자를 통해 초깃값을 설정 request영역에 변수를 저장 -->
	<c:set var = "personVar1" value='<%= new Person("박문수", 50) %>'
			scope="request" />
	<ul>
	<!-- request영역에 저장된 자바빈즈이 getter()를 통해 멤버변수의 값을 출력한다  -->
	...
	<!--  자바빈즈에 정의 setter()를 통해 조기화 한다.  값을 나중에 설정하려면
	target과 property속성을 사용합니다. target에는 변수를 property에는 멤버변수명을 지정한다-->
