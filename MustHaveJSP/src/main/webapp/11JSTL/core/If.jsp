...
<!-- 
 ...
 -->
	...
	<!-- 
		...
		통해 값에 대한 비교와 문자열에 대한 비교모드를 할 수 있다.
		여기서 사용한 eq는 Java의 comparTo()와 같이 사전순으로 문자열을
		비교한다.
	 -->
	<h4>문자열 비교과 else 구문 흉내내기</h4>
	<!-- 첫번째 if문은 false의 결과가 나온다. -->
	<c:if test="${ stirng eq 'Java' }" var="result2">
		문자열은 Java입니다. <br />
	</c:if>
	<!-- result2에 not을 붙여 반대의 조건을 만든다.(=>true)
	...
	<c:if test=" ${true} " var="result5">
		EL 양쪽에 빈 공백이 있는 경우 false <br /><!-- " ${true} " 이면 true. 출력된다. -->
	</c:if>
	result5 : ${ result5 } <br />
	...
	<!-- 
	 ...
	 <c:if test="${ not empty param.user }">
	 	...
	 	<!-- 
	 		아이디 패스워드를  입력했을 때만 일치여부를 판단한 후 결과를
	 		변수에 저장한다.
