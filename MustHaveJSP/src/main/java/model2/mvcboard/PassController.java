			...
			throws ServletException, IOException
	{
		/*
		 /mvcboard/pass.do?mode=delete&idx=122
		  ...
		  방법1 : request내장객체를 이용한다. => req.getParamete(파라미터명)
		...
		if(confirmed)
		{
			if(mode.equals("edit"))
			{
				/*
					...
					수정완료 할때까지 유지하고, 수정이 완료되면 제거할것이다. 여기에서
					저장한 패스워드는 수정을 위한 update쿼리문의 
					where절에 사용할 예정이다.
				*/
				...
				/*
				 	수정 페이지로 이동한다. 만약 아래 URL패턴을 이용해서 패스워드 검증 없이
				 	진입하였다면 세션에 저장된 패스워드가	없을것이므로
