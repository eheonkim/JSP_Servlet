...
/*
 	글쓰기로 페이지로 진입시에는 단순한 페이지 이동이므로 doGet() 요청을 
...
{	
			...
			throws ServletException, IOException
	{
		...
		// application 내장객체를 통해 web.xml에 초기화 파라미터를 얻어온다.
		...
		*/
		...
		/* 서버에 저장된 파일이 있는 경우에만 파일명 변경 처리를 한다.
		 	만약첨부하지 않았다면 아래코드는 실행하지 않는다.  */
		if (fileName != null)
		{
			// 날짜와 시간을 이요해서 파일명을 생성한다.
			String now = new SimpleDateFormat("yyyyMMdd_HmsS").format(new Date());
			//파일명의 마지막의 있는.(닷)의 인덱스를 찾은 후 확장자를 잘라낸다
			String ext = fileName.substring(fileName.lastIndexOf("."));
			// 파일명과 확장자를 연격ㄹ해서 새로운 파일명은 생성한다.
