...
{
	// 인수생성자에게는 application내장객체를 매개변수로 전달한다.
	...
	public int selectCount(Map<String, Object> map)
	{
		...
		if (map.get("seachWord") != null)
		{
			query += " WHERE " + map.get("seachField") + " "
	...
	public List<BoardDTO> selectList(Map<String, Object> map)
	{
		...
		// 게시판 목록은 출력 순서가 보장되야 하므로 Set컬렉션은 사용할 수 없고 List컬렉션을
		// 사용해야 한다.
	...
	public int insertWrite(BoardDTO dto)
	{
		...
		{
			/*
			 인파리터가 있는 동적쿼리문으로 insert문 작성 게시물의 일련번호는 시쿼스를 통해 
			 자동부여받고, 조회수의 경우에는 0을 입력한다.
			*/
	...
	public BoardDTO selectView(String num)
	{
		...
		try
		{
			...
			/*
				...
				next() 메서드는 ResultSet으로 반환된 게시물 확인해서 존재하멷 reuefn 
			 */
			if (rs.next())
			{
	...
	public List<BoardDTO> selectListPage(Map<String, Object> map)
	{
		...
		*/
		...
		// between을 통해 게시물의 구간을 결정할 수가 있다.
		query += "		ORDER BY num DESC "
			   + "	   ) TB "
			   + " ) "
		...
		*/
		try
		{
			// 인파라미터가 있는 쿼리문이므로 reapared객체를 생성한다.
			psmt = con.prepareStatement(query);
			// 인 파라미터를 설정한다. 구간의 시작과 끝을 계산한 값이다.
